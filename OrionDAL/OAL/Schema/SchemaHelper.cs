...
namespace OrionDAL.OAL.Schema
{
    public static class SchemaHelper
    {
        public static string Syncronize(Type baseEntityType, bool run)
        {
            System.Reflection.Assembly assembly = baseEntityType.Assembly;
            return Syncronize(baseEntityType, run, Transaction.Instance, assembly);
        }

        public static string Syncronize(Type baseEntityType, bool run, Transaction transaction, Assembly assembly)
        {
            StringBuilder str = new StringBuilder();
            if (OrionDAL.OAL.ConfigurationOrion.GetValue("DbType").ToString().Contains("My"))
                        ...
                        foreach (PropertyInfo property in props)
                        {
                            try
                            {
                                ...
                                bool isEntityPointer = property.PropertyType.IsGenericType
                                    && property.PropertyType.GetGenericTypeDefinition() == typeof(OrionDAL.ActiveRecord.SharpPointer<>);
